		...
		$.ajax({
			...
			success:	function(msg) 
			{
				...
				else if(object == "Program")
				{
					...
					if(objectId != null && objectId.length > 0)
					{	
						loadURLIntoId("/cat/program.jsp?program_id="+objectId,"#program");
					}
				...
				else if(object == "Course")
				{
					...
					if(courseId == null)
					{
						...
						document.location = "/cat/courseCharacteristicsWrapper.jsp?program_id="+programId+"&course_id="+courseId;
					}
					else
					{
						loadURLIntoId("/cat/courseCharacteristics.jsp?program_id="+programId+"&course_id="+courseId,"#CourseCharacteristicsDiv");
					}
				}
				...
				{
					var courseId = $("#course_id").val();
					loadURLIntoId("/cat/courseCharacteristics.jsp?link_id="+objectId+"&course_id="+courseId+"&program_id="+programId,"#CourseCharacteristicsDiv");
				}
				else if(object == "ProgramOutcome")
				{
					var programId = $("#program_id").val();
					loadURLIntoId("/cat/programOutcomes.jsp?program_id="+programId,"#programOutcomesDiv");
				}
				...
				else if(object == "InstructorAttributeValue")
				{
					...
					loadURLIntoId("/cat/courseOfferings.jsp?program_id="+programId+"&course_id="+courseId,"#courseOfferingsDiv");
				}
				...
				else if(object == "CourseAttributeValue")
				{
					...
					loadURLIntoId("/cat/programCourses.jsp?program_id="+programId,"#programCoursesDiv");
				}
				...
				else if(object=="ProgramOutcomeOrganizationOutcome")
				{
					...
					loadURLIntoId("/cat/outcomesMapping.jsp?program_id="+programId,"#outcomesMappingDiv");
				}
...
{
	if(!confirm("Are you sure you want to remove this program ? Any course associations, outcome links and characteristic links will be removed!"))
	{
		return;
	}
	$.ajax({
...
function removeProgramOutcome(program_id,link_id)
{
	...
	$.ajax({
		...
		success:	function(msg) 
		{
			...
			loadURLIntoId("/cat/programOutcomes.jsp?program_id="+program_id,"#programOutcomesDiv");
...
function removeProgramCourse(program_id,link_id)
{
	...
	$.ajax({
		...
		success:	function(msg) 
		{
			...
			loadURLIntoId("/cat/programCourses.jsp?program_id="+program_id,"#programCoursesDiv");
	...
	$.ajax({
		...
		success:	function(msg) 
		{
			...
			else
			{
				...
				loadURLIntoId("/cat/courseOfferings.jsp?program_id="+program_id+"&course_id="+courseId,"#courseOfferingsDiv");
			}
...
function loadSummaryBarGraphData(parameters)
{
	...
	$("#summaryAssessmentDiv").html('<img src="/cat/programCourseAssessmentData.jsp'+parameters+'&bogusParameter='+seconds+'"/>');
	$("#summaryAssessmentGroupsDiv").html('<img src="/cat/programAssessmentGroups.jsp'+parameters+'&bogusParameter='+seconds+'"/>');
	
}
function loadSummaryTeachingMethodBarGraphData(parameters)
{
	var seconds = new Date().getTime();
	$("#summaryTeachingMethodsDiv").html('<img src="/cat/programCourseTeachingMethodData.jsp'+parameters+'&bogusParameter='+seconds+'"/>');
}
function setProgramCourseContributionId(courseId)
{
	var programId = $("#programToSet").val();
	document.location="/cat/courseCharacteristicsWrapper.jsp?course_id="+courseId+"&program_id="+programId;
}
