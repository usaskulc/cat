...
String[] years = {"100-level", "200-level", "300-level" , "400-level"};
String[] yearCodes = {"firstYear", "secondYear", "thirdYear" , "fourthYear"};
...
for(LinkCourseProgram courseLink : courseLinks)
{
	...
	if(classification.getName().equalsIgnoreCase("required"))
	{
		...
		if(index < yearCodes.length)
		{
			...
			toAddTo.append("<input type=\"checkbox\" class=\"");
...
%><table id="uofs_table">
<tr><th colspan="4"><input type="checkbox" id="<%=current%>" value=".<%=current%>" onclick="gatherAssessmentData(<%=programId%>,this);">All <%=current%> Courses</th></tr>
<tr class="required">
<%for(int i = 0; i< yearCodes.length; i++)
{%>
	<th><input type="checkbox" class="<%=current%>" id="<%=yearCodes[i]%>" value=".<%=yearCodes[i]%>.<%=current%>" onclick="gatherAssessmentData(<%=programId%>,this);"><%=years[i]%></th>
<%
}
%></tr>
<tr>
...
<br><br>
<table>
<tr><th colspan="4"><input type="checkbox" id="<%=current%>" value=".<%=current%>" onclick="gatherAssessmentData(<%=programId%>,this);">All elective or optional Courses</th></tr>
<tr class="required">
<%for(int i = 0; i< yearCodes.length; i++)
{%>
	<th><input type="checkbox" class="<%=current%>" id="<%=yearCodes[i]%>" value=".<%=yearCodes[i]%>.<%=current%>" onclick="gatherAssessmentData(<%=programId%>,this);"><%=years[i]%></th>
...
<table>
	<tr><td><div id="summaryAssessmentDiv" style="width:600px;height:300px;">
		<img src="/cat/programCourseAssessmentData.jsp?program_id=<%=programId%>&time=<%=time%>" style="width:600px;height:300px;"></img>
	</div></td>
	<td>
	<div id="summaryTeachingMethodsDiv"  style="width:600px;height:300px;">
		<img src="/cat/programCourseTeachingMethodData.jsp?program_id=<%=programId%>&time=<%=time%>" style="width:600px;height:300px;"></img>
	</div></td>
	</tr>
	<tr><td><div id="summaryAssessmentGroupsDiv" style="width:600px;height:300px;">
		<img src="/cat/programAssessmentGroups.jsp?program_id=<%=programId%>&time=<%=time%>" style="width:600px;height:300px;"></img>
	</div></td>
